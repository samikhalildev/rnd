...
void draw() {
  ...
  for (int x = 0; x < particlesPerFrame; x++) {
    ...
    float pinch = 15+(sin(frameCount/50.0)*80);
  ...
  for (int i = allPixels.size()-1; i > -1; i--) {
    ...
    if (p.active) {
      ...
      ArrayList<PVector> flowField = flowFieldPresets.get(presetIndex);
      
      PVector direction = new PVector(flowField.get(index).x, flowField.get(index).y);
      direction.normalize();
    ...
    for (int j = 0; j < allPixels.size(); j++) {
      Pixel otherP = allPixels.get(j);
      
      if (p == otherP) {
        continue;
      }
      
      float d = dist(p.pos.x, p.pos.y, otherP.pos.x, otherP.pos.y);
      if (d < 15) {
        vertex(otherP.pos.x, otherP.pos.y);
        proximityCount++;
      }
    }
    
    stroke(min(proximityCount*3, 255), 255, 255, min(proximityCount*20, 255));
...
void mousePressed() {
  allPixels.clear();
  presetIndex += 1;
  if (presetIndex > flowFieldPresets.size()-1) {
    presetIndex = 0;
  }
}
